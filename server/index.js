...
const titles = {
  ...
  lahteetTitles: {
    ...
    en: ["Sources", "Makers?"],
    sv: ["Källförteckning", "Medverkande"],
  },
};
...
app.put("/subcategories/:id", async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [result] = await connection.query(
      "UPDATE subcategory SET image_path = ?, name = ?, name_en = ?, name_sv = ?, alateksti = ?, alateksti_en = ?, alateksti_sv = ?, vuosi = ? WHERE id = ?",
      [
        formattedImagePath,
        name,
        name_en,
...
app.post("/lahteet", async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const query =
      "INSERT INTO lahteet (tekijat, esiintyjat, esiintyjat_en, esiintyjat_sv, lahteet, subcategory_id, tekijat_sv) VALUES (?, ?, ?, ?, ?, ?, ?)";
    const values = [
      tekijat,
      esiintyjat,
      esiintyjat_en,
...
app.put("/lahteet/:subcategory_id", async (req, res) => {
  try {
    ...
    const newLahteetData = {
      ...
      tekijat_sv: req.body.tekijat_sv,
    };
    ...
    await connection.query(
      "UPDATE lahteet SET tekijat = ?, esiintyjat = ?, esiintyjat_en = ?, esiintyjat_sv = ?, lahteet = ?, tekijat_sv = ? WHERE subcategory_id = ?",
      [
        newLahteetData.tekijat,
        newLahteetData.esiintyjat,
        newLahteetData.esiintyjat_en,
        newLahteetData.esiintyjat_sv,
        newLahteetData.lahteet,   newLahteetData.tekijat_sv,
        subcategoryId,
      ...
      try {
        ...
        const esiintyjatColumn =
          selectedLanguage === "en"
            ...
            const tekijatColumn = selectedLanguage === "en" ? "tekijat_en": selectedLanguage === "sv" ? "tekijat_sv":"tekijat";
        ...
        const lahteetQuery = `SELECT ${tekijatColumn} AS tekijat, ${esiintyjatColumn} AS esiintyjat, lahteet FROM lahteet WHERE subcategory_id = ?`;
